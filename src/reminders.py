...
A reminder is {text, date (ISO), yearly, done}. One-offs fire once on their
date (and show as overdue after). Yearly ones (e.g. the April PPF top-up) use
only the month/day, so they roll forward on their own every year — no editing.
...
def new(text: str, on: str, yearly: bool = False) -> dict:
    return {"text": text, "date": on, "yearly": bool(yearly),
...
def effective_date(r: dict, today: date) -> date | None:
    ...
    occurrence (this year if still ahead, else next year). None if unparseable."""
    try:
...
def due(r: dict, today: date, horizon_days: int = 7) -> bool:
    if r.get("done") and not r.get("yearly"):
        return False
